...
import { listMedicineRecords, createMedicineRecord } from "@/lib/services/medicine";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = createMedicineRecordSchema.safeParse(json);
    if (!parsed.success) {
    ...
    const record = await createMedicineRecord(parsed.data);
