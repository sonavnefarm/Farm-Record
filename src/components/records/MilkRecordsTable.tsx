...
import { deleteMilkRecordAction } from "@/lib/actions/milk";
import type { MilkRecordWithAnimal } from "@/types/milk";

export function MilkRecordsTable({ records }: { records: MilkRecordWithAnimal[] }) {
  return (
    <div className="overflow-x-auto rounded-md border border-[var(--color-border)]">
      <table className="w-full text-left text-sm" aria-label="Milk records">
        <thead>
        ...
        <tbody>
          {records.map((record) => (
            ...
            >
              ...
              <td className="whitespace-nowrap px-4 py-2.5 font-mono text-[13px] font-medium text-[var(--color-text)]">
                {record.totalMilk.toFixed(2)}
              </td>
              <td className="whitespace-nowrap px-4 py-2.5 text-right">
                <div className="flex justify-end gap-1">
                  <Link
                    href={`/records/milk/${record.id}/edit`}
                    aria-label="Edit record"
                    className="rounded-md p-2 text-[var(--color-text-muted)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text)]"
                  >
                    <Pencil size={15} />
                  </Link>
                  <form action={deleteMilkRecordAction.bind(null, record.id, record.animal.id)}>
                    <button
                      type="submit"
